...
/**
 * Returns a viem WalletClient for all auth types (Privy, Web3Auth, MiniPay).
 *
 * Privy and MiniPay users get the wagmi-managed wallet client.
 * Web3Auth users get a viem client built from the Web3Auth EIP-1193 provider,
 * with the account field explicitly set so walletClient.account.address is
 * always defined (required by ClaimSDK and IdentitySDK).
 */
export function useUnifiedWalletClient() {
  const { data: wagmiWalletClient } = useWalletClient();
  const { provider: web3authProvider, isConnected: web3authConnected } = useWeb3Auth();
